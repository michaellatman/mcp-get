...
describe('ContinueAdapter', () => {
  ...
  const mockClientConfig: ClientConfig = {
    name: 'continue',
    type: 'continue',
    configPath: path.join(mockHomedir, '.continue', 'config.json')
  };

  beforeEach(() => {
    jest.clearAllMocks();
    adapter = new ContinueAdapter(mockClientConfig);
  });

  describe('getConfigPath', () => {
    it('should return the correct config path', () => {
      const configPath = adapter.getConfigPath();
      expect(configPath).toBe(path.join(mockHomedir, '.continue', 'config.json'));
    });
  });
  ...
  describe('validateConfig', () => {
    ...
    it('should not throw for valid websocket config', () => {
      const wsConfig: ServerConfig = {
        ...validConfig,
        transport: 'websocket'
      };
      expect(() => adapter.validateConfig(wsConfig)).not.toThrow();
    });
  });
});
